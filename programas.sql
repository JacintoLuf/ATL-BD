...
--sp que imprime todos os adultos ordenados pelos alunos que tem relaçao com eles
create proc agenda.adultos
as
	select id, aluno.nome, adulto.nome, phone from agenda.adulto
								join agenda.levantado on phone=levantado.parent_phone
								join agenda.aluno on id=id_aluno
	group by id, aluno.nome, adulto.nome, phone
	
exec agenda.adultos;

--sp que retorna alunos com problemas de saude ou alimentares
create proc agenda.problemas
as
	select * from agenda.aluno
	where aluno.problemas is not null or aluno.restricao is not null

exec agenda.problemas;

--sp todos os alunos e a que turma pertencem
create proc agenda.list_students_grade_year
as
	SELECT aluno.nome, turma.grau, turma.ano FROM agenda.aluno, agenda.turma, agenda.pertence_turma
	WHERE agenda.turma.id = agenda.pertence_turma.id_turma and agenda.aluno.id = agenda.pertence_turma.id_aluno;

exec agenda.list_students_grade_year

--Lista de alunos de uma visita de estudo
create proc agenda.alunos_visita @data date
as
	select id, aluno.nome, nasc, visita.nome, data
	from agenda.aluno join agenda.vai on id=id_aluno join agenda.visita on visita=data
	where visita = @data

exec agenda.alunos_visita '2020-06-02';
...
--Lista de alunos de visita de estudo
create proc agenda.alunos_visitas
as
	select id, aluno.nome, nasc, visita.nome, data
	from agenda.aluno join agenda.vai on id=id_aluno join agenda.visita on visita=data
	group by visita.nome, data, id, aluno.nome, nasc
	order by data desc

exec agenda.alunos_visitas;
...
--Lista de alunos e encarregados de educação
create proc agenda.alunos_encarregados
as
	select aluno.nome, nasc, adulto.nome, phone
	from agenda.aluno join agenda.encarregado on aluno.id=id_aluno join agenda.adulto on phone=parent_phone

exec agenda.alunos_encarregados;
...
--sp para saber a lista de alunos com atividade extracurricular
create proc agenda.extracurricular_students
as

    select tem.nome, aluno.nome
    from agenda.tem,agenda.aluno
    where tem.id_aluno = aluno.id group by tem.nome,aluno.nome

exec agenda.extracurricular_students

--Lista de atividade extracurricular e professor
alter proc agenda.profs_atividade
as
...
--sp para problemas de um certo aluno
create proc agenda.aluno_problema @name varchar(70), @nasc date
as
    declare @ide as int;
    exec agenda.student_id @name,@nasc,@ide OUTPUT;

    select problemas, medicacao, restricao, antipiretico from agenda.aluno
    where aluno.id=@ide

exec agenda.aluno_problema 'Pedro Henriques','2010-06-12';
...
--sp para saber turmas a que pertenceu certo aluno
create proc agenda.student_classes (@name varchar(70), @birthday date)
as
    declare @sid as int;
    exec agenda.student_id @name,@birthday,@sid OUTPUT;

    select aluno.nome, turma.ano, turma.grau from agenda.turma 
    join agenda.pertence_turma on turma.id = pertence_turma.id_turma
    join agenda.aluno on aluno.id = pertence_turma.id_aluno
    where aluno.id = @sid

exec agenda.student_classes 'Pedro Henriques','2010-06-12';
...
exec agenda.insert_adulto 965557014, 'Afonso Henriques', 'Rua do Castanhal Pousos', 965557014;
...
exec agenda.insert_professor 'Calisto Costa', 915555539, 'c.costa@gmail.com';--cuidado so o id é unico
...
--Insert parentesco(nome aluno data nasc,parent_phone, grau de parentesco)
--Insert encarregado(nome aluno data nasc, parent phone)
--Insert levantado(nome aluno data nasc,parent_phone)
...
--deletar adulto
create proc agenda.delete_adulto @phone numeric(9)
as
	if(select count(*) from agenda.parente where parent_phone=@phone) > 0
		begin
			delete from agenda.parente where parent_phone=@phone
		end
	if(select count(*) from agenda.levantado where parent_phone=@phone) > 0
		begin
			delete from agenda.levantado where parent_phone=@phone
		end
	if(select count(*) from agenda.encarregado where parent_phone=@phone) > 0
		begin
			delete from agenda.encarregado where parent_phone=@phone
		end
	delete from agenda.adulto where phone = @phone

--deletar levantado
create proc agenda.delete_levantado @nome varchar(70), @nasc date, @phone numeric(9)
as
	declare @ide int
	exec agenda.student_id @nome, @nasc, @ide OUTPUT;
	delete from agenda.levantado where id_aluno = @ide and parent_phone = @phone
...
--deletar encarregado
create proc agenda.delete_encarregado @nome varchar(70), @nasc date
as
	declare @ide int
	exec agenda.student_id @nome, @nasc, @ide OUTPUT;
	delete from agenda.encarregado where id_aluno = @ide

--deletar aluno de uma turma
create proc agenda.delete_aluno_turma @nome varchar(70), @nasc date
as
	declare @ide int
	exec agenda.student_id @nome, @nasc, @ide OUTPUT;
	delete from agenda.pertence_turma where id_aluno = @ide
...
--deletar aluno de uma visita
create proc agenda.delete_aluno_visita @nome varchar(70), @nasc date
as
	declare @ide int
	exec agenda.student_id @nome, @nasc, @ide OUTPUT;
	delete from agenda.vai where id_aluno = @ide
